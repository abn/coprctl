...
| **package** | none | A named source definition inside a project, not an RPM. |
...
| **module** | none | Module *building* was removed upstream; the `additional_modules` project-chroot field still round-trips as `module_toggle` but has no build effect. |
