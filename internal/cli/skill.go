...
func buildMainSkill(root *cobra.Command, _ *App) string {
	...
	md.WriteString("- When a command fails, run `coprctl doctor` first: it distinguishes config, auth, and network problems in one shot.\n")
	md.WriteString("- `coprctl chroot list` uses a cached catalog when the instance is unreachable; a warning on stderr means it fell back to cache.\n")
	md.WriteString("- Group-owned projects use `@group/project`; the group must first be activated in the Copr web UI at `/groups/list/my`.\n")
	md.WriteString("- coprctl adds an activation hint when a `@group` owner returns not found (404).\n\n")
